# Terraform — Beginner-Level Interview Questions & Answers

Below are professional, thorough, and interview-ready answers to each of the beginner-level Terraform questions. Answers are written in a formal, corporate tone with appropriate technical terminology and practical context.
...
**Key differences:**  
- Terraform maintains a state file and performs a diffing/plan stage before apply.
- Terraform is multi-cloud and focuses on resource lifecycle management.
- Ansible runs tasks to bring systems to a desired state (idempotency but not the same state model).
- CloudFormation is tightly integrated with AWS and offers deep service support and native drift detection but lacks cross-cloud portability.
...
**Answer:**  
Providers are plugins that implement the API interactions for a given service or platform. Providers expose resources and data sources to Terraform, enabling it to create, read, update, and delete objects in that platform.
...
- `helm` (Helm charts)
- Third-party providers like `datadog`, `github`, `cloudflare`
...
> **Interview Tip:** Reference any modules you’ve used: official Terraform Registry modules like `terraform-aws-vpc`, or internal company modules.
...
**Answer:**  
`terraform init` initializes a working directory containing Terraform configuration. It downloads and installs provider plugins, initializes backend configuration, installs modules, sets up the workspace, and minimally validates configuration syntax. It must be run before other Terraform commands in a new or changed working directory.
...
**Answer:**  
`terraform refresh` updates the state file to reflect the real-world resource attributes by querying providers. It can be used when resources may have changed outside Terraform (drift) and you want to synchronize state before planning.  
> Note: `refresh` is usually implicit during planning in newer Terraform versions; explicit refresh is less common.
...
**Answer:**  
Variables are referenced using the `var` namespace, e.g., `var.instance_type`.
...
1. **Install Terraform** and configure AWS credentials (env vars, shared credentials file, or profile).
2. **Create configuration files:** provider block (`provider "aws" { region = "ap-south-1" }`), resource block (`resource "aws_instance" "web" { ... }`), variables, outputs as needed.
3. **Initialize:** run `terraform init` to download the AWS provider.
4. **Plan:** run `terraform plan` to review changes.
5. **Apply:** run `terraform apply` (or `terraform apply -auto-approve`) to create instance.
6. **Inspect outputs:** use `terraform output` to get public IP, instance ID.
7. **Teardown (if needed):** `terraform destroy`.
...
**Answer:**  
Define an output block referencing the resource attribute.
...
**Answer:**  
Terraform will detect that the resource defined in the state/configuration no longer exists remotely. `terraform plan` will show the resource as to be created (because Terraform will attempt to reconcile the desired state with reality by recreating the missing resource).  
- If the deletion was intentional and you want Terraform to stop managing it, use `terraform state rm` to remove it from the state.
- If you want to adopt an existing resource into state, you can use `terraform import`.